...
void LineMatching::DebugShow(const int &debug_show,
    ...
    // for (int i = 0; i < lines_ref_num; i++){
    //     if(status[i] == 0) continue;

    //     if(line_ref_to_line_cur[i] < 0) continue;
    //     int a = (rand() % 200);
	// 	int b = (rand() % 200 + 99);
	// 	int c = (rand() % 200) - 50;
	// 	if (a > 100 || a < 0)
	...
	// 	int radius = 5;
	// 	//左片
    //     // cv::Point(int(lines_cur[i].line_endpoint[0]) + width, int(lines_cur[i].line_endpoint[1]))
    //     int id = line_ref_to_line_cur[i];
        
	// 	int sx1 = int(lines_ref[i].line_endpoint[0]);
	// 	int sy1 = int(lines_ref[i].line_endpoint[1]);
    //     int ex1 = int(lines_ref[i].line_endpoint[2]);
	// 	int ey1 = int(lines_ref[i].line_endpoint[3]);
    //     cv::line(img_show, cv::Point(sx1,sy1), cv::Point(ex1,ey1),cv::Scalar(a, b, c), 3,8,0);

	// 	cv::circle(img_show, cv::Point(sx1, sy1), radius, cv::Scalar(0, 255, 255), 1, 4, 0);
	// 	cv::line(img_show, cv::Point(sx1 - radius - 2, sy1), cv::Point(sx1 + radius + 2, sy1), cv::Scalar(0, 255, 255), 1, 8, 0);
	// 	cv::line(img_show, cv::Point(sx1, sy1 - radius - 2), cv::Point(sx1, sy1 + radius + 2), cv::Scalar(0, 255, 255), 1, 8, 0);

	// 	cv::circle(img_show, cv::Point(ex1, ey1), radius, cv::Scalar(0, 255, 255), 1, 4, 0);
	// 	cv::line(img_show, cv::Point(ex1 - radius - 2, ey1), cv::Point(ex1 + radius + 2, ey1), cv::Scalar(0, 255, 255), 1, 8, 0);
	// 	cv::line(img_show, cv::Point(ex1, ey1 - radius - 2), cv::Point(ex1, ey1 + radius + 2), cv::Scalar(0, 255, 255), 1, 8, 0);

	// 	//右片
    //     int sx2 = int(lines_cur[id].line_endpoint[0]) + img_ref.cols;
	// 	int sy2 = int(lines_cur[id].line_endpoint[1]) ;
    //     int ex2 = int(lines_cur[id].line_endpoint[2])+ img_ref.cols;
	// 	int ey2 = int(lines_cur[id].line_endpoint[3]) ;
    //     cv::line(img_show, cv::Point(sx2,sy2), cv::Point(ex2,ey2),cv::Scalar(a, b, c), 3,8,0);

        
	// 	cv::circle(img_show, cv::Point(sx2, sy2), radius, cv::Scalar(0, 255, 255), 1, 4, 0);
	// 	cv::line(img_show, cv::Point(sx2 - radius - 2, sy2), cv::Point(sx2 + radius + 2, sy2), cv::Scalar(0, 255, 255), 1, 8, 0);
	// 	cv::line(img_show, cv::Point(sx2, sy2 - radius - 2), cv::Point(sx2, sy2 + radius + 2), cv::Scalar(0, 255, 255), 1, 8, 0);

	// 	cv::circle(img_show, cv::Point(ex2, ey2), radius, cv::Scalar(0, 255, 255), 1, 4, 0);
	// 	cv::line(img_show, cv::Point(ex2 - radius - 2, ey2), cv::Point(ex2 + radius + 2, ey2), cv::Scalar(0, 255, 255), 1, 8, 0);
	// 	cv::line(img_show, cv::Point(ex2, ey2 - radius - 2), cv::Point(ex2, ey2 + radius + 2), cv::Scalar(0, 255, 255), 1, 8, 0);
    //     	// //连接
	// 	cv::line(img_show, cv::Point(sx1,sy1), cv::Point(sx2, sy2),cv::Scalar(a, b, c), 1,8,0);
    //     // cv::line(img_show, cv::Point(ex1,ey1), cv::Point(ex2, ey2),cv::Scalar(a, b, c), 1,8,0);
    // }
