...
void Estimator::setParameter()
{
    ...
    lineProjectionFactor::sqrt_info = line_factor * Matrix2d::Identity();
    vpProjectionFactor::sqrt_info =  vp_factor* Matrix2d::Identity();
    //  vpProjectionFactor::sqrt_info =   5.0*Matrix2d::Identity();
...
void Estimator::processImage(const map<int, vector<pair<int, Eigen::Matrix<double, 5, 1>>>> &image, const map<int, vector<pair<int, Eigen::Matrix<double, 8, 1>>>> &lines, const std_msgs::Header &header)
{
    ...
    if(ESTIMATE_EXTRINSIC == 2)
    {
        ...
        if (frame_count != 0)
        {
            ...
            double td = 0.0;
            initial_ex_time.CalibrationExTime(corres,pre_integrations[frame_count]->delta_q,header.stamp.toSec(),pre_integrations[frame_count]->sum_dt,td);
        //         ROS_INFO("time offest %d",td);
    ...
    // if(ESTIMATE_EXTRINSIC == 3){
    //     // if (frame_count != 0)
    //     double td = 0.0;
    //     {
    //         vector<pair<Vector3d, Vector3d>> corres = f_manager.getCorresponding(frame_count - 1, frame_count);
    //         // initial_ex_time.CalibrationExTime(corres,pre_integrations[frame_count]->delta_q,header.stamp.toSec(),pre_integrations[frame_count]->sum_dt,td);
    //         // while(initial_ex_time.CalibrationExTime(f_manager.getCorresponding(frame_count - 1, frame_count),pre_integrations[frame_count]->delta_q,header.stamp.toSec(),pre_integrations[frame_count]->sum_dt,td));
    //         // {
    //             // ROS_INFO("calibrating extrinsic time");
    //             // ROS_INFO("time offest %d",td);
    //         // }
    //     }
    //     ROS_INFO("time offest %d",td);
...
// p
//Eigen::Matrix<double, 5, 1>
void Estimator::processImage(const map<int, vector<pair<int, Eigen::Matrix<double, 5, 1>>>> &image,  const std_msgs::Header &header)
{
    ROS_DEBUG("new image coming ------------------------------------------");
    ROS_DEBUG("Adding feature points %lu", image.size());
  
    if(f_manager.addFeatureCheckParallax(frame_count, image))       // 对检测到的特征进行存放处理
        marginalization_flag = MARGIN_OLD;
    else                                                                   // 当视差较小时，比如静止，marg 新的图像帧
        marginalization_flag = MARGIN_SECOND_NEW;
...
void  Estimator::onlyLineOpt()
{
    ...
    for (auto &it_per_id : f_manager.linefeature)
    {
        ...
        for (auto &it_per_frame : it_per_id.linefeature_per_frame)
        {
            ...
            // if(it_per_frame.lineobs_vp(3) == 1.0)
            // {

            //     Vector4d vp_line_obs = it_per_frame.lineobs_vp;                     
            //     Vector3d vp_obs(vp_line_obs(0),vp_line_obs(1),vp_line_obs(2));
            //     vpProjectionFactor *f_vp = new vpProjectionFactor(vp_obs);    

            //     problem.AddResidualBlock(f_vp, loss_function,
            //                          para_Pose[imu_j],
            //                          para_Ex_Pose[0],
            //                          para_LineFeature[feature_index]);
