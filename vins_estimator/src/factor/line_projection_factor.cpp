...
Eigen::Matrix2d vpProjectionFactor::sqrt_info;
double lineProjectionFactor::sum_t;


bool vpProjectionFactor::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
{
    ...
    Eigen::Vector3d d_c(line_c(3,0),line_c(4,0),line_c(5,0));

    Matrix<double, 2, 1> d_c_2d(d_c(0)/d_c(2), d_c(1)/d_c(2));

    Matrix<double, 2, 1> vp_2d(obs_i(0)/obs_i(2), obs_i(1)/obs_i(2));

    double v3_inv = 1/obs_i(2);

    double v1v3_inv = vp_2d(0);
    double v2v3_inv = vp_2d(1);
    
    Eigen::Map<Eigen::Vector2d> residual(residuals);
    residual(0) = (d_c_2d(0) - vp_2d(0));
    residual(1) = (d_c_2d(1) - vp_2d(1));
    // sqrt_info = Eigen::
    // if(abs(residual(0))>=15.0 || abs(residual(1))>=15.0)
    //      residual = 0.1*sqrt_info * residual;
    // else
        residual = sqrt_info * residual;
    // if(residual(0)>=50.0 || residual(1)>=50.0)
    //      residual = 0.0000001*sqrt_info * residual;
    // else
        // residual = sqrt_info * residual;
    #if true
    // std::cout << "dc:"<<d_c_2d.transpose() <<"\n";
    // std::cout << "vc:"<<vp_2d.transpose() <<"\n";
    // std::cout << "residual:"<<residual.transpose() <<"\n";
    if (jacobians)
    {

        Eigen::Matrix<double, 2, 3> jaco_e_l(2, 3);
        jaco_e_l << ( -1/obs_i(2) ), (0.0 ), v1v3_inv,
                (0.0 ), (-1/obs_i(2) ), v2v3_inv;

        jaco_e_l = sqrt_info * jaco_e_l;

        Eigen::Matrix<double, 3, 6> jaco_l_Lc(3, 6);
        jaco_l_Lc.setZero();
        jaco_l_Lc.block(0,3,3,3) = Eigen::Matrix3d::Identity();
