...
{
public:
    lineFeaturePerFrame(const Eigen::Matrix<double, 8, 1> &line)
    {
        lineobs(0,0) = line(0,0);
        lineobs(1,0) = line(1,0);
        lineobs(2,0) = line(2,0);
        lineobs(3,0) = line(3,0);

        lineobs_vp(0,0) = line(4,0);
        lineobs_vp(1,0) = line(5,0);
        lineobs_vp(2,0) = line(6,0);

        lineobs_vp(3,0) = line(7,0);

    }

    Vector4d lineobs;   // 每一帧上的观测
    Vector4d lineobs_vp;   // 每一帧上的观测
    // Vector4d lineobs_R;
    double z;
    bool is_used;
    double parallax;
...
class FeatureManager
{
  public:
    ...
    bool addFeatureCheckParallax(int frame_count, const map<int, vector<pair<int, Matrix<double, 5, 1>>>> &image, const map<int, vector<pair<int, Eigen::Matrix<double, 8, 1>>>> &lines);
    bool addFeatureCheckParallax(int frame_count, const map<int, vector<pair<int, Eigen::Matrix<double, 5, 1>>>> &image);
