...
std::string readParameters(ros::NodeHandle &n)
{
    ...
    USE_FEATURE_SELECTOR = fsSettings["use_feature_selector"];
    MAX_FEATURES = fsSettings["max_features"];
    INIT_THREASHOLD = fsSettings["init_threshold"];
    USE_GROUND_TRUTH = static_cast<int>(fsSettings["use_ground_truth_hgen"]) != 0;
